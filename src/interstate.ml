...
module Network = struct
  ...
  module Connection = struct
    ...
    let of_string s =
      let cities = List.tl_exn (String.split s ~on:',') in

      let rec createPairs cityList =
        match List.length cityList with
        | 1 -> []
        | _ ->
          let curCity = List.hd_exn cityList in
          let pairList = List.concat_map ~f:(fun city -> [curCity, city; city, curCity]) (List.tl_exn cityList) in
          pairList @ createPairs (List.tl_exn cityList)
      in
      createPairs cities
    ;;
  end
...
(* ignore (input_file : File_path.t); ignore (output_file : File_path.t);
   printf !"Done! Wrote dot file to %{File_path}\n%!" output_file] *)
