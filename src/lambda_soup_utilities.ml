...
let get_first_item_of_all_unordered_lists contents : string list =
  ...
  $$ "ul"
  |> to_list
  |> List.map ~f:(fun ul -> ul $ "li"|> texts |> String.concat ~sep:"" |> String.strip)
;;
...
let get_first_item_of_second_unordered_list contents : string =
  ...
  let ulLists = parse contents
  $$ "ul"
  |> to_list
  |> List.map ~f:(fun ul -> ul $ "li"|> texts |> String.concat ~sep:"" |> String.strip) in

  List.hd_exn (List.tl_exn ulLists)
;;
